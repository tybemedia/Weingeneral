...
const fetchProducts = async () => {
  ...
  try {
    ...
    if (!wineCategory) {
      throw new Error('Weinkategorie nicht gefunden');
    }
    
    console.log('Wine category found:', wineCategory);
    categoryId.value = wineCategory.id;
    
    // Fetch products with a timeout
    const productsData = await Promise.race([
      getProductsByCategory(wineCategory.id),
    ...
    if (!productsData || productsData.length === 0) {
      console.warn('No products found for category:', wineCategory.id);
      products.value = [];
    } else {
    ...
    <ClientOnly>
      <v-container class="py-16">
        ...
        <div 
          ...
          :enter="{ opacity: 1, y: 0, transition: { duration: 500 } }"
        >
          <v-icon size="64" color="grey-lighten-1" class="mb-4">mdi-wine-bottle-outline</v-icon>
