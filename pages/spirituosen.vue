...
const fetchProducts = async () => {
  ...
  try {
    ...
    // Find the spirits category
    const spiritsCategory = categories.find(cat => 
      cat.name.toLowerCase().includes('spirituosen') || 
      cat.slug.toLowerCase().includes('spirituosen')
    );
    
    if (!spiritsCategory) {
      throw new Error('Spirituosen-Kategorie nicht gefunden');
    }
    
    console.log('Spirits category found:', spiritsCategory);
    categoryId.value = spiritsCategory.id;
    
    // Fetch products with a timeout
    const productsData = await Promise.race([
      getProductsByCategory(spiritsCategory.id),
    ...
    if (!productsData || productsData.length === 0) {
      console.warn('No products found for category:', spiritsCategory.id);
      products.value = [];
    } else {
...
<template>
  <div class="product-page">
    ...
    >
      <v-img
        src="https://images.pexels.com/photos/613182/pexels-photo-613182.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
        height="400"
        cover
        class="header-image"
      >
        <div class="header-overlay d-flex align-center justify-center">
          <v-container>
            ...
            >
              Entdecken Sie unsere Auswahl an erlesenen Spirituosen
            </p>
          </v-container>
        </div>
      </v-img>
    ...
    <ClientOnly>
      <v-container class="py-16">
        ...
        <div 
          ...
          :enter="{ opacity: 1, y: 0, transition: { duration: 500 } }"
        >
          <v-icon size="64" color="grey-lighten-1" class="mb-4">mdi-glass-cocktail</v-icon>
