...
const { data: initialProducts } = await useAsyncData('equipment-products', async () => {
  try {
    const categories = await getCategories();
    const ausstattungCategory = categories.find(cat => 
      cat.name.toLowerCase().includes('ausstattung') || 
      cat.slug.toLowerCase().includes('ausstattung')
    );
    
    if (ausstattungCategory) {
      categoryId.value = ausstattungCategory.id;
      const productsData = await getProductsByCategory(ausstattungCategory.id);
      loading.value = false;
      return productsData;
    }
    loading.value = false;
      ...
      >
        <div class="header-overlay d-flex align-center justify-center">
          <v-container>
            ...
            >
              Professionelle Ausstattung für Ihre Weinbar
            </p>
          </v-container>
        </div>
      </v-img>
